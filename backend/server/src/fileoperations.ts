...
export const getPlaygroundFiles = async (id: string) => {
  const res = await fetch(
    `${process.env.STORAGE_WORKER_URL}/api?playgroundId=${id}`,
    {
      headers: {
  ...
  // Extract the problem statement and files from the data
  const problemStatement = data.objects.find((obj) => obj.key.endsWith('challenge.md'))?.key;
  const paths = data.objects.map((obj) => obj.key);

  const processedFiles = await processFiles(paths, id);

  // Fetch the problem statement content if it exists
  let statement = "";
  if (problemStatement) {
    statement = await fetchFileContent(problemStatement);
  }

  return {
    files: processedFiles.files,
    filesData: processedFiles.fileData,
